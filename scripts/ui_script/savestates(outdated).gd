...
#const ironman_texture = preload("res://assets/UI/UI_Icon/2773431.png")
...
func _on_PlayChar1_pressed():
	if charslot_dict["Charslot1"][0] != "empty":
		...
		if savefile.file_exists("user://save_file"+ charslot_dict["Charslot1"][0] +".dat"):
			...
			get_tree().change_scene("res://scenes/UI/SkillSelection.tscn")
		else:
			savefile.open("user://save_file"+ charslot_dict["Charslot1"] +".dat", File.WRITE)
			savefile.close()
			CharacterSave.character_name = charslot_dict["Charslot1"][0]
			CharacterSave.ironman = charslot_dict["Charslot1"][1]
			$SFXStreamPlayer.play()
			yield(get_tree().create_timer(0.5), "timeout")
			get_tree().change_scene("res://scenes/UI/CharacterCreator.tscn")
